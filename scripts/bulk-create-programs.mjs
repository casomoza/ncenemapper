/**
 * Bulk-create 30 new programs from the 2026-2027 catalog.
 * Run: node scripts/bulk-create-programs.mjs
 * Uses the Supabase anon key (read+insert is permitted via RLS for authenticated-ish anon).
 */
import fs from "fs";

const URL_BASE = "https://exuretutssbihksumeyo.supabase.co/rest/v1";
const KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImV4dXJldHV0c3NiaWhrc3VtZXlvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkyMDczMDEsImV4cCI6MjA5NDc4MzMwMX0.Na8dxC3_B3ea9LeOzkKjwYBRA1LBPNZAfat5zUHrO1U";
const HEADERS = {
  apikey: KEY,
...
async function dbGet(table, params = "") {
  const r = await fetch(`${URL_BASE}/${table}?${params}`, { headers: HEADERS });
  return r.json();
}
...
// ── Load catalog course details ──────────────────────────────────────────────
const catalogRaw = fs.readFileSync("./src/lib/catalog-courses.json", "utf8");
const catalogCourses = JSON.parse(catalogRaw); // array
const byCode = {};
...
function assignSemesters(courses) {
  ...
  if (n <= 3) {
    ...
    for (let i = 0; i < n; i++) {
      const q2 = Math.floor(i / q);
      const map = [{ year: 1, semester: "Fall" }, { year: 1, semester: "Spring" }, { year: 2, semester: "Fall" }, { year: 2, semester: "Spring" }];
      sems.push(map[Math.min(q2, 3)]);
    }
  }
...
// ── BA Core – shared across 5 BA concentration programs ──────────────────────
const BA_CORE = [
...
const PROGRAMS = [
  // ── BUSINESS & MANAGEMENT ─────────────────────────────────────────────────
  {
    ...
    outcomes: [
      ...
      "Apply accounting principles related to payroll, cost, income tax, and computerized accounting",
    ...
    courses: [
      ...BA_CORE,
      { code: "ACC-1B", category: "core" },
      { code: "ACC-40", category: "elective", optional: true },
  ...
  },
  {
    ...
    outcomes: [
      ...
      "Apply basic business and accounting calculations and analyses",
      "Analyze the law as it pertains to business organizations",
      "Explain and develop the marketing mix and analyze marketing mix variables",
    ],
    courses: [
      ...BA_CORE,
      { code: "ACC-1B", category: "elective", optional: true },
      { code: "BUS-40", category: "elective", optional: true },
      { code: "BUS-80", category: "elective", optional: true },
      { code: "MAG-51", category: "elective", optional: true },
      { code: "MAG-53", category: "elective", optional: true },
      { code: "MKT-20", category: "elective", optional: true },
    ],
  },
  {
    ...
    outcomes: [
      ...
      "Contribute knowledge needed by multidisciplinary teams to integrate and exceed end user expectations",
      "Analyze, prepare, file and process claims when freight disputes arise",
      "Identify 3rd party logistics provider and client needs in negotiations, bidding, and contracts",
    ],
    courses: [
      ...BA_CORE,
      { code: "BUS-80", category: "core" },
      { code: "BUS-85", category: "elective", optional: true },
      { code: "BUS-86", category: "elective", optional: true },
      { code: "BUS-87", category: "elective", optional: true },
      { code: "BUS-90", category: "elective", optional: true },
    ],
  },
  {
    ...
    total_units: 30,
    description:
      "Prepares individuals to plan, organize, direct, and control the functions and processes of a firm or organization with an emphasis on people as the most important asset. Prepares students seeking management positions to be better candidates for promotion and those already in management to improve effectiveness.",
    outcomes: [
      "Use technology to analyze business decisions and enhance business communications",
      "Apply sound management practices",
      "Analyze and apply appropriate managerial practices in ethics, human resources, and quality management",
      "Apply basic business and accounting calculations and analyses",
    ],
    courses: [
      ...BA_CORE,
      { code: "MAG-44", category: "core" },
      { code: "MAG-53", category: "elective", optional: true },
      { code: "MAG-56", category: "elective", optional: true },
      { code: "MAG-60", category: "elective", optional: true },
      { code: "BUS-48", category: "elective", optional: true },
    ],
  },
  {
    ...
    total_units: 30,
    description:
      "Prepares individuals to develop, buy, sell, appraise, and manage real property. Includes instruction in land use development policy, real estate law, real estate marketing procedures, agency management, brokerage, property inspection and appraisal, real estate investing, and property management.",
    outcomes: [
      ...
      "Calculate real estate taxes and solve basic real estate mathematics problems",
      "Explain and evaluate methods of financing real estate purchases and securing loans",
    ],
    courses: [
      ...BA_CORE,
      { code: "RLE-80", category: "elective", optional: true },
      { code: "RLE-81", category: "elective", optional: true },
      { code: "RLE-82", category: "elective", optional: true },
      { code: "RLE-83", category: "elective", optional: true },
      { code: "RLE-84", category: "elective", optional: true },
      { code: "RLE-85", category: "elective", optional: true },
    ],
  },
  {
    ...
    courses: [
      { code: "CAT-1A", category: "core" },
      { code: "CAT-3", category: "core" },
      { code: "CAT-31", category: "core" },
      { code: "CAT-51", category: "core" },
      { code: "CAT-90", category: "core" },
      { code: "CAT-93", category: "core" },
      { code: "CAT-98A", category: "core" },
      { code: "CAT-98B", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Develop a business plan outlining the viability and sustainability of an idea",
      "Create a value proposition and test market assumptions for a business idea",
      "Apply standard accounting practices and evaluate finance opportunities",
    ],
    courses: [
      { code: "ENP-50", category: "core" },
      { code: "ENP-51", category: "core" },
      { code: "ENP-52", category: "core" },
      { code: "ENP-53", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Develop a comprehensive business plan outlining the viability and sustainability of an idea",
      "Apply standard accounting practices and evaluate finance opportunities",
      "Create a Business Model Canvas showcasing main components and deliver a compelling presentation",
    ],
    courses: [
      { code: "ENP-50", category: "core" },
      { code: "ENP-51", category: "core" },
      { code: "ENP-53", category: "core" },
      { code: "ENP-54", category: "core" },
      { code: "ENP-55", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Demonstrate an understanding of the entrepreneurial process from idea generation to commercialization",
      "Analyze and evaluate potential business ideas for marketability and success",
      "Create and evaluate a comprehensive business plan",
      "Outline and construct steps needed to create an effective social marketing campaign for a small business",
    ],
    courses: [
      { code: "BUS-30", category: "core" },
      { code: "BUS-14", category: "core" },
      { code: "BUS-12", category: "core" },
      { code: "BUS-13", category: "core" },
    ],
  },
  {
    ...
    total_units: 18,
    description:
      "Designed for those interested in starting their own business, exposing students to the basics of entrepreneurship. Utilizes the Business Model Canvas technique and provides critical insights into the basics of founding-team formation, the Gig economy, legal business structures, e-commerce, and networks.",
    outcomes: [
      "Develop a business plan outlining the viability and key elements in establishing business partnerships",
      "Apply standard accounting practices and evaluate finance opportunities",
      "Create a Business Model Canvas and deliver a compelling presentation",
      "Demonstrate the principles of the Gig Economy and recognize different types of entrepreneurial categories",
    ],
    courses: [
      { code: "ENP-50", category: "core" },
      { code: "ENP-51", category: "core" },
      { code: "ENP-53", category: "core" },
      { code: "ENP-54", category: "core" },
      { code: "ENP-70", category: "core" },
      { code: "ENP-71", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Demonstrate an understanding of the entrepreneurial process from idea generation to commercialization",
      "Apply accounting and bookkeeping for small business principles",
      "Analyze and evaluate various funding sources for small businesses",
      "Outline and evaluate the legal steps and issues necessary for opening a small business",
    ],
    courses: [
      { code: "BUS-30", category: "core" },
      { code: "ACC-55", category: "core" },
      { code: "BUS-31", category: "core" },
      { code: "BUS-33", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Compare roles and objectives of the logistics disciplines",
      "Understand how logistics functions interact to efficiently use total personnel, facilities, and equipment",
    ...
    courses: [
      { code: "BUS-80", category: "core" },
      { code: "BUS-82", category: "core" },
      { code: "BUS-83", category: "core" },
      { code: "BUS-85", category: "core" },
      { code: "BUS-86", category: "core" },
      { code: "BUS-87", category: "core" },
      { code: "BUS-90", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Analyze ethical and procedural problems that arise in real estate transactions",
      "Discuss and evaluate real estate marketing and sales techniques",
      "Explain and evaluate methods of financing and evaluating real estate",
      "Demonstrate knowledge of state and federal statutes and regulations affecting real estate sales",
    ],
    courses: [
      { code: "RLE-80", category: "core" },
      { code: "RLE-81", category: "core" },
      { code: "RLE-82", category: "elective", optional: true },
      { code: "RLE-83", category: "elective", optional: true },
      { code: "RLE-85", category: "elective", optional: true },
    ],
  },
  {
    ...
    outcomes: [
      "Prepare federal and state income tax returns for individuals, sole proprietorships, and other business entities",
      "Conduct tax research on client issues using manual and computerized methods",
      "Evaluate and propose strategies that minimize income tax obligations",
    ],
    courses: [
      { code: "ACC-67", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Use GAAP guidelines to review and interpret financial documents",
      "Calculate pricing models for mark-ups, profit margins, discounts, and sinking funds",
      "Prepare and deliver effective oral and written communications through multiple modes",
      "Analyze the effectiveness of marketing decisions and use marketing principles to assess market potential",
    ],
    courses: [
      { code: "ACC-1A", category: "core" },
      { code: "BUS-20", category: "core" },
      { code: "BUS-22", category: "core" },
      { code: "CIS-1A", category: "core" },
      { code: "MAG-56", category: "core" },
      { code: "MAG-44", category: "core" },
      { code: "MAG-53", category: "core" },
      { code: "MKT-20", category: "core" },
      { code: "MKT-42", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Perform a variety of accounting skills such as journalizing, posting, and double entry accounting",
      "Use accounting software to prepare financial statements and analyze problems",
      "Recognize the role of ethics in accounting",
    ],
    courses: [
      { code: "ACC-65", category: "core" },
      { code: "ACC-1A", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Analyze and evaluate payroll principles as defined by Social Security Act and laws relating to payment of wages",
      "Analyze and solve problems associated with calculation and reporting of payroll",
      "Accurately apply accounting principles to computerized and manual payroll systems",
    ],
    courses: [
      { code: "ACC-62", category: "core" },
      { code: "ACC-1A", category: "core" },
    ],
  },
  ...
  // ── HUMAN & PUBLIC SERVICES ───────────────────────────────────────────────
  {
    ...
    outcomes: [
      ...
      "Analyze and demonstrate advanced knowledge in Constitutional criminal law, civil rights, and rules of evidence",
      "Demonstrate a proficient level of knowledge in evidence identification, collection, preservation, and chain of custody",
      "Understand basic and intermediate forensic science principles and techniques",
    ],
    courses: [
      { code: "ADJ-2", category: "core" },
      { code: "ADJ-3", category: "core" },
      { code: "ADJ-4", category: "core" },
      { code: "ADJ-12", category: "core" },
      { code: "ADJ-13", category: "core" },
      { code: "ADJ-14", category: "core" },
      { code: "ADJ-27", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Develop, implement, and evaluate developmentally appropriate curriculum for children who are typical and atypical",
      "Develop and apply appropriate practices that respect the cultural diversity of young children and their families",
      "Integrate an educational philosophy into classroom practices that reflects a personal belief supportive of theoretical principles",
      "Develop and implement a system of ongoing observational practices that adapts to the evolving needs of children",
    ],
    courses: [
      { code: "EAR-19", category: "core" },
      { code: "EAR-20", category: "core" },
      { code: "EAR-24", category: "core" },
      { code: "EAR-25", category: "core" },
      { code: "EAR-26", category: "core" },
      { code: "EAR-28", category: "core" },
      { code: "EAR-30", category: "core" },
      { code: "EAR-42", category: "core" },
      { code: "EAR-23", category: "elective", optional: true },
      { code: "EAR-33", category: "elective", optional: true },
      { code: "EAR-40", category: "elective", optional: true },
      { code: "EAR-43", category: "elective", optional: true },
    ],
  },
  {
    ...
    courses: [
      { code: "EAR-20", category: "core" },
      { code: "EAR-24", category: "core", optional: true },
      { code: "EAR-28", category: "core", optional: true },
      { code: "EAR-42", category: "core", optional: true },
    ],
  },
  {
    ...
    total_units: 34,
    description:
      "Appropriate for students interested in working as an assistant or paraprofessional in early intervention, early childhood special education, and community child development programs serving children with special needs. Provides practical skills and on-site training for employment in the field of Early Childhood Intervention.",
    outcomes: [
      "Demonstrate an understanding of family function and structure along with familial need for information and support that respects diverse cultures",
      "Demonstrate basic knowledge of laws and regulations pertaining to and protecting children with disabilities",
      "Describe typical child development milestones and identify strengths and special needs of the child",
      "Demonstrate understanding of the purpose and intent of an inclusive environment supporting children with disabilities",
    ],
    courses: [
      { code: "EAR-19", category: "core" },
      { code: "EAR-20", category: "core" },
      { code: "EAR-24", category: "core" },
      { code: "EAR-28", category: "core" },
      { code: "EAR-40", category: "core" },
      { code: "EAR-41", category: "core" },
      { code: "EAR-42", category: "core" },
      { code: "EAR-43", category: "core" },
      { code: "EAR-46", category: "core" },
      { code: "EAR-26", category: "elective", optional: true },
      { code: "EAR-33", category: "elective", optional: true },
  ...
  // ── MATH / ENGINEERING / CS / GAME DEV ────────────────────────────────────
  {
    ...
    outcomes: [
      "Design and create images used for printed media in advertising web design",
      "Apply techniques to create and modify artwork using vector-based and bit-mapped programs",
      "Integrate text and graphics in a document layout program to create professional-quality full-color documents",
      "Demonstrate knowledge of design principles in advertising and layout design, type, and lettering applications",
    ],
    courses: [
      { code: "CIS-66", category: "core" },
      { code: "CIS-72B", category: "core" },
      { code: "CIS-78A", category: "core" },
      { code: "CAT-78B", category: "core" },
      { code: "CIS-79", category: "core" },
      { code: "CIS-81", category: "core" },
      { code: "CIS-59", category: "core" },
      { code: "ART-22", category: "core" },
      { code: "ART-39", category: "core" },
      { code: "CIS-44", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Build, test, and document analog and digital prototypes based on theory-driven design techniques",
      "Apply the principles of modern game development through the creation of creative assets and supporting materials",
      "Contribute to working games and prototypes requiring team management, effective planning, and communication",
      "Create modern portfolio demonstrating viable capability utilizing games and class projects",
    ],
    courses: [
      { code: "GAM-1", category: "core" },
      { code: "GAM-2", category: "core" },
      { code: "GAM-3A", category: "core" },
      { code: "GAM-3B", category: "core" },
      { code: "GAM-4A", category: "core" },
      { code: "GAM-5A", category: "core" },
      { code: "GAM-6A", category: "core" },
      { code: "GAM-7", category: "core" },
      { code: "GAM-8", category: "core" },
    ],
  },
  {
    ...
    outcomes: [
      "Build simple to intermediate game systems using logic and programming using modern game development software",
      "Apply the principles of modern game development through the creation of creative assets and supporting materials",
      "Contribute to working games and prototypes requiring team management, effective planning, and communication",
      "Create modern portfolio demonstrating viable capability utilizing games and class projects",
    ],
  ...
  },
  {
    ...
    outcomes: [
      "Apply the principles of modern game development through the creation of creative assets and supporting materials",
      "Contribute to working games and prototypes requiring team management, effective planning, and communication",
      "Develop content that contributes and adds value to games projects or portfolio",
      "Create modern portfolio demonstrating viable capability utilizing games and class projects",
    ],
    courses: [
      { code: "GAM-1", category: "core" },
  ...
  },
  {
    ...
    outcomes: [
      ...
      "Apply the principles of modern game development through the creation of creative assets",
      "Contribute to working games and prototypes requiring team management, effective planning, and communication",
      "Create modern portfolio demonstrating viable capability utilizing games and class projects",
    ],
    courses: [
  ...
  },
  {
    ...
    outcomes: [
      "Build modeled, textured, rigged, and animated 3D game assets using modern software and techniques",
      "Apply the principles of modern game development through the creation of creative assets and supporting materials",
      "Contribute to working games and prototypes requiring team management, effective planning, and communication",
      "Create modern portfolio demonstrating viable capability utilizing games and class projects",
    ],
  ...
  // ── VISUAL & PERFORMING ARTS ─────────────────────────────────────────────
  {
    ...
    courses: [
      { code: "MIS-1A", category: "core" },
      { code: "MIS-1B", category: "core" },
      { code: "MIS-1C", category: "core" },
      { code: "MUS-3", category: "core" },
      { code: "MUS-93", category: "core" },
      { code: "MIS-3", category: "elective" },
      { code: "MIS-2", category: "elective", optional: true },
      { code: "MIS-4", category: "elective", optional: true },
      { code: "MIS-7", category: "elective", optional: true },
  ...
  },
  {
    ...
    courses: [
      ...
      { code: "MUS-3", category: "core" },
      { code: "MUS-93", category: "core" },
      { code: "MUS-39", category: "core" },
      { code: "MUS-79", category: "core" },
      { code: "MUS-41", category: "core" },
      { code: "MIS-3", category: "elective", optional: true },
      { code: "MIS-7", category: "elective", optional: true },
...
async function run() {
  ...
  for (const prog of PROGRAMS) {
    ...
    const existing = await dbGet("programs", `select=id&slug=eq.${encodeURIComponent(slug)}`);
    if (Array.isArray(existing) && existing.length > 0) {
      console.log(`⏭  Skipping (exists): ${prog.name}`);
      skipped++;
      continue;
    }

    // Insert program
    const { data: progResult, error: progErr } = await dbInsert("programs", {
      slug,
      name: prog.name,
      degree_type: prog.degree_type,
      cluster: prog.cluster,
      total_units: prog.total_units,
      description: prog.description,
      outcomes: prog.outcomes,
    });

    if (progErr) {
      console.error(`❌ Error inserting program "${prog.name}":`, progErr.message);
      errors.push(prog.name);
      continue;
    }

    const newProg = Array.isArray(progResult) ? progResult[0] : progResult;

    // Build course rows with semester assignment + catalog details
    const enriched = prog.courses.map((c) => {
      ...
      return {
        ...
        prerequisite: det.prerequisite,
      };
    });

    const withSemesters = assignSemesters(enriched);

    const courseRows = withSemesters.map((c, i) => ({
    ...
    const { error: courseErr } = await dbInsert("courses", courseRows);

    if (courseErr) {
      console.error(`❌ Error inserting courses for "${prog.name}":`, courseErr.message);
      errors.push(`${prog.name} (courses)`);
    } else {
      console.log(`✅ Created: ${prog.name} (${courseRows.length} courses)`);
      created++;
    }
  }
