...
type CourseSpec = { code: string; category: string; optional?: boolean };
type ProgramSpec = {
  name: string;
  degree_type: string;
  cluster: string;
...
const PROGRAMS: ProgramSpec[] = [
  ...
  },
  {
    ...
    cluster: "School of Business & Management",
    total_units: 30,
    description: "Prepares students for entry into or career growth within the logistics industry. Focuses on integrated logistics covering warehousing, transportation, service contracting, purchasing, and global logistics.",
  ...
  },
  {
    ...
    cluster: "School of Business & Management",
    total_units: 18,
    description: "Designed for those interested in starting their own business. Utilizes the Business Model Canvas technique and provides critical insights into founding-team formation, the Gig economy, legal business structures, e-commerce, and networks.",
  ...
  },
  {
    ...
    cluster: "School of Business & Management",
    total_units: 4,
    description: "U.S. and California income tax principles and tax return preparation as it relates to individuals, sole proprietorships, and other business entities. Certified by the California Tax Education Council (CTEC) as fulfilling the 60-hour qualifying education requirement for becoming a Registered Tax Preparer.",
  ...
  },
  {
    ...
    total_units: 21,
    description: "Provides a strong academic and skill-building pattern of coursework to prepare participants with academic and technical knowledge in forensic and criminal investigative techniques used within crime scene investigations.",
  ...
  },
  {
    ...
    cluster: "School of Human & Public Services",
    total_units: 31,
    description: "Provides an educational and practical foundation for students interested in working with children from infancy through third grade. Fulfills required child development coursework for the state-issued Child Development Permit.",
  ...
  },
  {
    ...
    cluster: "School of Human & Public Services",
    total_units: 34,
    description: "Appropriate for students interested in working as an assistant or paraprofessional in early intervention, early childhood special education, and community child development programs serving children with special needs.",
  ...
  },
  {
    ...
    total_units: 27,
    description: "Designed for students who wish to pursue training in desktop publishing. Training focuses on using a computer to design page layouts, develop presentations, and create advertising campaigns using digital images and printable media.",
  ...
  },
  {
    ...
    cluster: "School of Math, Engineering, Computer Science & Game Development",
    total_units: 27,
    description: "Students will be well versed in the process by which games and game assets are designed and created, with a special emphasis on how they are programmatically constructed. Students will be capable of building complete simple games and intermediate game systems.",
  ...
  },
  {
    ...
    cluster: "School of Math, Engineering, Computer Science & Game Development",
    total_units: 27,
    description: "Students will be well versed in the process by which games and game assets are designed and created, with a special emphasis on how visual development influences game design and aesthetic. Students will be capable of designing and visually articulating a range of asset types.",
  ...
  },
  {
    ...
    total_units: 37,
    description: "Designed to provide students with the knowledge and skills necessary for producing popular music and engineering in the recording studio as well as for live sound. Students become proficient on a DAW, gain experience recording and producing music on digital and analog devices.",
  ...
  },
  {
    ...
    cluster: "School of Visual & Performing Arts",
    total_units: 36,
    description: "Designed to provide students with the knowledge and skills necessary for studio recording and live performance in the commercial music industry. Students become proficient on an instrument or voice, gain experience as an ensemble member, and study the fundamentals of music.",
...
function assignSemesters(courses: CourseSpec[]) {
  const n = courses.length;
  const sems: { year: number; semester: string }[] = [];
  if (n <= 3) {
...
function AdminImportPage() {
  ...
  const setStatus = (name: string, status: Status, msg?: string) => {
    setStatuses((prev) => ({ ...prev, [name]: status }));
    if (msg) setMessages((prev) => ({ ...prev, [name]: msg }));
  };
  ...
  async function runImport() {
    ...
    for (const prog of PROGRAMS) {
      ...
      setStatus(prog.name, "running");

      const { data: existing } = await supabase.from("programs").select("id").eq("slug", slug).maybeSingle();
      if (existing) {
        setStatus(prog.name, "skipped", "Already exists");
        continue;
      }

      const { data: newProg, error: progErr } = await supabase
        .from("programs")
        .insert({ slug, name: prog.name, degree_type: prog.degree_type, cluster: prog.cluster, total_units: prog.total_units, description: prog.description, outcomes: prog.outcomes })
        .select()
        .single();

      if (progErr || !newProg) {
        setStatus(prog.name, "error", progErr?.message ?? "Insert failed");
        continue;
      }

      const withSems = assignSemesters(prog.courses);
      const courseRows = withSems.map((c, i) => ({
      ...
      if (courseErr) {
        setStatus(prog.name, "error", courseErr.message);
      } else {
        setStatus(prog.name, "done", `${courseRows.length} courses added`);
      }
    }
  ...
  const done = counts.filter((s) => s === "done").length;
  const skipped = counts.filter((s) => s === "skipped").length;
  const errors = counts.filter((s) => s === "error").length;

  const clusterGroups: Record<string, ProgramSpec[]> = {};
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 px-6 py-10">
        ...
        <div className="mb-8">
          ...
          <h1 className="font-serif text-3xl font-semibold text-foreground">Import Programs from Catalog</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Creates {PROGRAMS.length} new program pathway maps from the 2026–2027 Norco College catalog. Skips any that already exist.
          </p>
        </div>
        ...
        {!running && !finished && (
          <div className="mb-8 rounded-lg border border-border bg-card p-6">
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              {Object.entries(clusterGroups).map(([cluster, progs]) => (
                <div key={cluster}>
                  <p className="text-xs font-medium text-muted-foreground">{cluster.replace("School of ", "")}</p>
                  <p className="mt-0.5 text-2xl font-semibold text-foreground">{progs.length}</p>
            ...
            <button
              onClick={runImport}
              className="mt-6 inline-flex items-center gap-2 rounded-md bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground hover:bg-burgundy"
            >
              <Download className="h-4 w-4" />
              Import all {PROGRAMS.length} programs
            </button>
          </div>
        )}

        {(running || finished) && (
          <div className="mb-6 flex items-center gap-4 rounded-lg border border-border bg-card px-5 py-4">
            {running ? (
            ...
            <div className="text-sm">
              ...
              {skipped > 0 && <span className="ml-2 text-muted-foreground">{skipped} skipped</span>}
              {errors > 0 && <span className="ml-2 text-destructive">{errors} errors</span>}
            </div>
            {finished && (
              <Link to="/admin" className="ml-auto text-sm text-primary hover:underline">
                View all programs →
              </Link>
            )}
          </div>
          ...
          {Object.entries(clusterGroups).map(([cluster, progs]) => (
            <div key={cluster}>
              ...
              <div className="divide-y divide-border rounded-lg border border-border bg-card">
                {progs.map((p) => {
                  const s = statuses[p.name] ?? "pending";
                  return (
                    <div key={p.name} className="flex items-center gap-3 px-4 py-3">
                      <div className="w-5 shrink-0">
                        {s === "pending" && <Clock className="h-4 w-4 text-muted-foreground/40" />}
                        {s === "running" && <Loader2 className="h-4 w-4 animate-spin text-primary" />}
                        {s === "done" && <CheckCircle2 className="h-4 w-4 text-green-600" />}
                        {s === "skipped" && <CheckCircle2 className="h-4 w-4 text-muted-foreground" />}
                        {s === "error" && <XCircle className="h-4 w-4 text-destructive" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="truncate text-sm font-medium text-foreground">{p.name}</p>
                        <p className="text-xs text-muted-foreground">
                          {p.degree_type} · {p.total_units} units · {p.courses.length} courses
                        </p>
                      </div>
                      {messages[p.name] && (
                        <span className={`shrink-0 text-xs ${s === "error" ? "text-destructive" : "text-muted-foreground"}`}>
                          {messages[p.name]}
                        </span>
                      )}
                    </div>
